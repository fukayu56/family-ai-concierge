...
// Load root .env (if any) then server/.env for local runs.
dotenv.config();
...
logSafe(`OpenAI Model: ${openAIModel}`);

/** Lazy so missing key does not crash module import (health/spots still work). */
...
// CORS:
// - Same-origin production (Expo Web + API on one Vercel host) does not need cross-origin CORS.
// - Local Expo Web (localhost:8081) → API (localhost:3001) remains cross-origin and must be allowed.
// - ALLOWED_ORIGINS (comma-separated) is still supported for previews / extra origins.
// - Never use cors() with unrestricted allow-all.
...
function isAllowedOrigin(origin: string): boolean {
  ...
  // When ALLOWED_ORIGINS is unset on Vercel, allow same-deployment preview/prod hosts.
  if (process.env.VERCEL === '1' && allowedOriginsFromEnv.length === 0) {
    try {
...
app.post('/api/recommendations', async (req: Request, res: Response) => {
  ...
  if (isServerlessRuntime) {
    console.log(
      `prompt length: ${prompt.length} chars (full prompt omitted in production)`
    );
  } else {
    console.log(prompt);
  }
  ...
  if (isServerlessRuntime) {
    console.log(
      `plans=${apiResponse.plans.length}, relaxedPlans=${apiResponse.relaxedPlans?.length ?? 0} (body omitted in production)`
    );
  } else {
...
// Minimal error middleware — keep existing JSON error contract (no stack / secrets).
app.use(
  (
    err: unknown,
    _req: Request,
